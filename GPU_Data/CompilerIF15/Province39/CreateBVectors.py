...
def writeX(fnum, data, tstep):
    fname = "f" + str(fnum) + "VecB_t" + str(tstep) + ".txt"
...
def main(maindir):
    ...
    for i in range(1,40):
        datafile = indir + str(i) + "/VectorB.txt"
    ...
    t = 1
    while True:
        sysfiles = [x for x in os.listdir(maindir) if ("VecB_t" + str(t) 
            + ".txt") in x]
        ...
        if len(sysfiles) != 39:
            ...
            if len(out) != LINES:
                ...
                outfile = maindir + "SysVecB_t" + str(t) + ".txt"
